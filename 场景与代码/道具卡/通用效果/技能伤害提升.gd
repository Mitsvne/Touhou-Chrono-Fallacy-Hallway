class_name EffectSkillDamageUp
extends CardEffect
...
func apply_passive(player: CharacterBody2D) -> void:
	if not player or not player.character_data or not player.character_data.current_skill:
		return
	var hits=player.character_data.current_skill.hits
	for i in range(hits.size()):
		hits[i].damage_multiplier *= (1.0 + damage_add_percent)
		hits[i].damage_multiplier +=  damage_add_value
		print("卡牌被动：", player.character_data.character_name, " 的技能",i+1,"段倍率提升至 ", hits[i].damage_multiplier*100,"%")
...
func remove_passive(player: CharacterBody2D) -> void:
	...
	for i in range(hits.size()):
		hits[i].damage_multiplier -= damage_add_value
		hits[i].damage_multiplier /= (1.0 + damage_add_percent)
		print("卡牌卸载：", player.character_data.character_name, " 的技能第",i+1,"段倍率恢复至 ", hits[i].damage_multiplier*100,"%")
